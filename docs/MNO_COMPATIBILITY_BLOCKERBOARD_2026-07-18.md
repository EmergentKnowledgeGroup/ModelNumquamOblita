...
**Overall status:** **RELEASE CANDIDATE VERIFIED. Implementation, local artifact proof, and the GitHub cross-platform artifact matrix are green; final exact-head review, merge, tag, and publication remain release-process steps.**
...
- `MNO-COMPAT-013` passed on the exact code candidate. Merge only after the final documentation-only head also passes CI and CodeRabbit; tag and publish only from the merged `main` commit.
