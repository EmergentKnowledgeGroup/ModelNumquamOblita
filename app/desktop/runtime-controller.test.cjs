...
test('parseShellCliArgs reads explicit shell overrides', () => {
  const parsed = parseShellCliArgs(['--memories', '/tmp/store.sqlite3', '--episodes', '/tmp/cards.json', '--port', '8450', '--python', 'py', '--config', '/tmp/policy.json', '--repo-root', '/repo', '--smoke-exit-when-ready', '--boot-timeout-ms', '45000']);
  ...
  assert.equal(parsed.config, '/tmp/policy.json');
...
test('buildRuntimeLaunchPlan keeps runtime launcher bounded and explicit', () => {
  ...
  const plan = buildRuntimeLaunchPlan({ repoRoot: '/repo', runtimeManifest: manifest, host: '127.0.0.1', port: 8123, memories: '/tmp/store.sqlite3', episodes: '/tmp/cards.json', config: '/tmp/policy.json' });
  ...
  assert.ok(plan.args.includes(path.resolve('/tmp/policy.json')));
