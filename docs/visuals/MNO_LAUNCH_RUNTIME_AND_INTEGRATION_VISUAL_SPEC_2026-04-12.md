...
Use this authority ordering in both views: human-reviewed canonical truth, evidence atom, consolidated provisional, observed provisional. `review_apply` is a human-only capability: applying a proposal creates a `human_reviewed=false` evidence atom, not canonical truth. WSS and STM are helper state, never evidence.
